...
def Recognition():
    ...
    # 读取图片
    pic = None
    url = '991c67b5c23e3d135374b74eb8e4caa.jpg'
    try:
        pic = cv2.imread(url)
    except:
        return "Wrong Url"
    ...
    for item in data:
        ...
        if item['type'] == 'buttom':
            ...
            rowHeight -= 80
            pic = paddingdrawithcolor(pic,f"结果{str(resultIndex)}: 状态：{'正常' if status==0 else '异常'}",(0,rowHeight),status)
        ...
        # 表识别
            ...
            pic = paddingdrawithcolor(pic,f"结果{str(resultIndex)}: 状态：{'正常' if status==0 else '异常'}",status)
            pic = boxdraw(pic, [int(x) for x in item['location'].split(',')], status, f"结果{str(resultIndex)}")
            resultIndex += 1
