...
def yolov8m(img, \
    ...
    # 定义匹配阈值
    threshold = 200  # 这个阈值需要根据具体情况进行调整
    ...
    for boxIndex, detection_box in enumerate(boxesList[:6]):
        cv2.rectangle(img, (int(detection_box[0]),int(detection_box[1])), (int(detection_box[2]),int(detection_box[3])), (boxIndex*30,0,255-boxIndex*30), 2)
